...
const PartsCart: React.FC<IPageProps> = ({ cart, parts }: IPageProps) => {
  ...
  return (
    <div className="w-full lg:max-w-xs max-w-4xl lg:ml-4 mx-auto lg:my-0 z-10 mt-4 lg:min-h-screen px-5 ">
      ...
      <div className="flex lg:flex-col  flex-row flex-wrap justify-evenly w-full mt-2 bg-white p-6">
        {cart &&
          Object.values(cart).map((item, i) => {
            ...
            return (
              <a key={i} href={`/platform#${item[0].part_id}`}>
                <div className="m-3 px-3 pt-4 pb-4 flex-grow w-72 lg:w-full bg-gray-bg hover:translate-x-2 hover:shadow-lg transition-all rounded-lg shadow-sm border-accent border-2 relative">
                  <div>
                    ...
                    <div className="pb-3">
                      <div className="text-base leading-relaxed font-bold text-accent-dark">
                        {part?.name}
                      </div>
